...
#if defined(CONFIG_TSHARK2TABE) || defined(CONFIG_GRANDPRIME3G_VE)|| defined(CONFIG_COREPRIME3G_VE) || defined(CONFIG_TSHARK2J2_3G) || defined(CONFIG_TIZENZ3_3G) || defined(CONFIG_GRANDPRIME_DTV)
...
#else
	val = BITS_SMPL_ENABLE(SMPL_MODE_DISABLE_SET);
	ci_adi_write_fast(ANA_REG_GLB_SMPL_CTRL0, val, 1);
#endif	/* CONFIG_TIZEN */
...
#endif
	/*
	 * To avoid GPU lockup issue (Bug 413283, 428056),
	 *  disable GPU clk auto gating on SC7730 board
	 */
	gpu_clk_auto_gate_disable();
