...
static struct s_ldo_ctl_info s_ldo_ctl_data[] = {
    {LDO_LDO_USB,        ANA_REG_GLB_LDO_PD_CTRL,    BIT(8)},
    {LDO_LDO_EMMCCORE,   ANA_REG_GLB_LDO_DCDC_PD,    BIT(7)},
    {LDO_LDO_EMMCIO,     ANA_REG_GLB_LDO_DCDC_PD,    BIT(6)},
    {LDO_LDO_SDIO3,      ANA_REG_GLB_LDO_PD_CTRL,    BIT(0)},
    {LDO_LDO_SIM0,       ANA_REG_GLB_LDO_PD_CTRL,    BIT(1)},
    {LDO_LDO_SIM1,       ANA_REG_GLB_LDO_PD_CTRL,    BIT(2)},
    {LDO_LDO_SIM2,       ANA_REG_GLB_LDO_PD_CTRL,    BIT(3)},
    {LDO_LDO_CAMMOT,       ANA_REG_GLB_LDO_PD_CTRL,    BIT(7)},
    {LDO_LDO_WIFIPA,       ANA_REG_GLB_LDO_PD_CTRL,    BIT(11)},
};

static int __LDO_IDX(LDO_ID_E ldo_id)
{
   int i=0;
   for(i=0; i< ARRAY_SIZE(s_ldo_ctl_data);i++)
   {
      if(ldo_id == s_ldo_ctl_data[i].id)
	  	return i;
   }
   return -1;
}
...
#else
    int i = __LDO_IDX(ldo_id);

	if (i < 0)
		return LDO_ERR_ERR;

	ANA_REG_OR(s_ldo_ctl_data[i].reg, s_ldo_ctl_data[i].bit);
...
LDO_ERR_E LDO_TurnOnLDO(LDO_ID_E ldo_id)
{
	...
	ANA_REG_BIC(s_ldo_ctl_data[i].reg, s_ldo_ctl_data[i].bit);
