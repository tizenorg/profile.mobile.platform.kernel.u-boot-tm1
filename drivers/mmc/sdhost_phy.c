...
#elif defined(CONFIG_SC8830) || (defined CONFIG_SC9630)
...
PUBLIC void SDHOST_SD_POWER (SDHOST_HANDLE sdhost_handler,SDHOST_PWR_ONOFF_E on_off)
{
    ...
    if (POWR_ON == on_off)
    {
		...
		{
			LDO_TurnOnLDO(LDO_LDO_EMMCCORE);
			LDO_TurnOnLDO(LDO_LDO_EMMCIO);
		}
		...
		/*
			...
			sharkl no need open vddsdio.
			if need,turn on vddsdio
		...
		*/
		...
		/*
			...
			LDO_TurnOnLDO(LDO_LDO_SDIO1);
		*/
			#if defined(CONFIG_SP9830IEA_5M_H100)
				LDO_TurnOnLDO(LDO_LDO_SDIO1);
				LDO_TurnOnLDO(LDO_LDO_SDIO0);
			#else
				LDO_TurnOnLDO(LDO_LDO_SDIO0);
    ...
    else  // power off
    {
		...
		if (sdhost_handler->slotNo == SDHOST_SLOT_7)
		{
			LDO_TurnOffLDO(LDO_LDO_EMMCCORE);
			LDO_TurnOffLDO(LDO_LDO_EMMCIO);
		}
		else if (sdhost_handler->slotNo == SDHOST_SLOT_6)
		{
...
#else // no defined(PLATFORM_SC8800G)
...
PUBLIC SDHOST_HANDLE SDHOST_Register (SDHOST_SLOT_NO slot_NO,SDIO_CALLBACK fun)
{
    ...
    // select slot 0
#if defined (CONFIG_SC8825) || defined(CONFIG_SPX15)||defined(CONFIG_SPX30G) || (defined CONFIG_SC9630)/*CONFIG_SPX15 must be before CONFIG_SC8830*/
	sdio_port_ctl[slot_NO].open_flag = TRUE;
	#if defined(CONFIG_ARCH_SCX20L) || defined(CONFIG_SPX20)
...
//    SDHOST_Slot_select(slot_NO); if necessary
	...
	sdio_port_ctl[slot_NO].baseClock = 36000000;
#endif
...
#define REG_PIN_EMMC_CLK     REG_PIN_NFREN
#define REG_PIN_EMMC_CMD     REG_PIN_NFRBN
#define REG_PIN_EMMC_D0      REG_PIN_NFWPN
#define REG_PIN_EMMC_D1      REG_PIN_NFD11
#define REG_PIN_EMMC_D2      REG_PIN_NFD14
#define REG_PIN_EMMC_D3      REG_PIN_NFD7
...
#define REG_PIN_EMMC_D6      REG_PIN_NFCLE
#define REG_PIN_EMMC_D7      REG_PIN_NFALE
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_CLK)= 0x80010;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_CMD)= 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D0) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D1) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D2) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D3) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D4) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D5) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D6) = 0x80090;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D7) = 0x80090;
#else	/* TIZEN */
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_CLK)= 0x00;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_CMD)= 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D0) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D1) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D2) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D3) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D4) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D5) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D6) = 0x80;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D7) = 0x80;
#if 0
...
#define REG_PIN_EMMC_D7      REG_PIN_NFALE
			...
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_CLK)= 0x82011;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_CMD)= 0x83094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D0) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D1) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D2) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D3) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D4) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D5) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D6) = 0x3094;
			REG32(CTL_PIN_BASE+REG_PIN_EMMC_D7) = 0x3094;
#else
	/* Other chip,do nothing*/
#endif
#endif /* CONFIG_FPGA */
#endif
            {
...
#if defined(CONFIG_SPX15)|| (defined CONFIG_SC9630)
...
#if 0
				if (sdio_base_clk >= SDIO_BASE_CLK_256M)
				{
					REG32(REG_AP_CLK_SDIO0_CFG) |=  3;
					clk = SDIO_BASE_CLK_256M;
				}
