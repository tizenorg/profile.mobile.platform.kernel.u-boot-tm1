...
static void s6e8aa5x01_test_key(struct panel_spec *self, uint8_t enable)
{
...
static int32_t s6e8aa5x01_check_mtp(struct panel_spec *self)
{
	...
	mipi_set_cmd_mode_t mipi_set_video_mode =
	...
	for (i = 0; i < 3; i++) {
		...
		if (mtp_offset[30] != 2 || mtp_offset[31] != 3 ||
			mtp_offset[32] != 2)
			printk("%s: fail to read mtp offset[%d]\n",
					__func__, i);
		else
			break;
	}

	s6e8aa5x01_test_key(self, 0x0);
	mipi_eotp_set(0, 0);
	mipi_set_video_mode();

	save_mtp_offset_to_kernel(mtp_offset);
...
static int32_t s6e8aa5x01_check_elvss(struct panel_spec *self)
{
	...
	save_elvss_offset_to_kernel(elvss_offset);
...
static int32_t s6e8aa5x01_check_hbm(struct panel_spec *self)
{
	...
	save_hbm_offset_to_kernel(hbm_g);
...
static struct panel_operations lcd_s6e8aa5x01_mipi_operations = {
	...
	.panel_check_mtp = s6e8aa5x01_check_mtp,
	.panel_check_elvss = s6e8aa5x01_check_elvss,
	.panel_check_hbm = s6e8aa5x01_check_hbm,
};
