				...
				disk_partition_t * info)
{
	...
	if (is_gpt_valid(dev_desc, GPT_PRIMARY_PARTITION_TABLE_LBA,
			&(gpt_head), &pgpt_pte) != 1) {
		debug("%s: *** ERROR: Invalid Main GPT ***\n", __FUNCTION__);
		if(is_gpt_valid(dev_desc, dev_desc->lba -1, &(gpt_head), &pgpt_pte) != 1){
			debug("%s: *** ERROR: Invalid alternate GPT ***\n", __FUNCTION__);
			return -1;
...
int get_partition_num_by_name_efi(block_dev_desc_t *dev_desc, const char *partition_name)
{
	...
	int ret = -1;
	unsigned int i, j, partition_nums = 0;
	int part_name_len = (72 / sizeof(efi_char16_t));

	if (!dev_desc || !partition_name) {
		debug("%s: Invalid Argument(s)\n", __FUNCTION__);
		return -1;
	}
	...
	partition_nums = le32_to_int(gpt_head.num_partition_entries);
	for (i = 0; i < partition_nums; i++) {
		for (j = 0; j < part_name_len; j++) {
			if (partition_name[j] != pgpt_pte[i].partition_name[j])
				break;
			if (partition_name[j] == 0 && pgpt_pte[i].partition_name[j] == 0) {
				ret = i + 1;
				goto free;
			}
		}
			...
			gpt_header * pgpt_head, gpt_entry ** pgpt_pte)
{
	...
	//debug("Enter is_gpt_valid 0x%16Lx \n", lba);
	...
	if (le64_to_int(pgpt_head->signature) != GPT_HEADER_SIGNATURE) {
		debug("GUID Partition Table Header signature is wrong:"
			"0x%llX != 0x%llX\n",
	...
	if (calc_crc32 != le32_to_int(crc32_backup)) {
		debug("GUID Partition Table Header CRC is wrong:"
			"0x%08lX != 0x%08lX\n",
	...
	if (le64_to_int(pgpt_head->my_lba) != lba) {
		debug("GPT: my_lba incorrect: %llX != %llX\n",
	...
	*pgpt_pte = alloc_read_gpt_entries(dev_desc, pgpt_head);
	if (*pgpt_pte == NULL) {
		debug("GPT: Failed to allocate memory for PTE\n");
		return 0;
	}
	...
	if (calc_crc32 != le32_to_int(pgpt_head->partition_entry_array_crc32)) {
		debug("GUID Partition Table Entry Array CRC is wrong:"
			"0x%08lX != 0x%08lX\n",
					 ...
					 gpt_header * pgpt_head)
{
	...
	if (count == 0 || pte == NULL) {
		debug("%s: ERROR: Can't allocate 0x%X bytes for GPT Entries\n",
			__FUNCTION__, count);
		return NULL;
	}
	...
	if (dev_desc->block_read (dev_desc->dev,
		...
		debug("*** ERROR: Can't read GPT Entries ***\n");
		free(pte);
		return NULL;
	}
	return pte;
