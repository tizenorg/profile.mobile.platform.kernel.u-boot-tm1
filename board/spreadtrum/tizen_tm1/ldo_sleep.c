...
void init_ldo_sleep_gr(void)
{
	...
	//vddsdio 3.3v
	ANA_REG_SET(ANA_REG_GLB_LDO_V_CTRL4, 0x3CD2);
	...
	*************************************************/
	...
	/* Bug 364866_IQ imbalance patch */
	reg_val = CHIP_REG_GET(REG_AON_APB_BB_BG_CTRL);
	reg_val &= ~(0xF << 16);
	reg_val |= (0x4 << 16);
	CHIP_REG_SET(REG_AON_APB_BB_BG_CTRL,reg_val);
